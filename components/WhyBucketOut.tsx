        ...
        {[...cards, ...cards].map((card, idx) => (
          <article key={`${card.title}-${idx}`} className="deal-card">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[0.9rem] font-medium uppercase tracking-wide text-[#47558a]">{card.category}</p>
                <h3 className="mt-1 text-[2rem] font-semibold leading-tight text-[#122265]">{card.title}</h3>
              </div>
            </div>
            <div className="mt-8 flex items-center justify-between">
              <span className="inline-flex items-center rounded-full bg-white px-4 py-2 text-[1.6rem] font-semibold text-[#14256a]">
                5 Stars
              </span>
              <strong className="text-[2.2rem] font-bold text-[#14256a]">{card.price}</strong>
            </div>
          </article>
        ))}
      </div>
      ...
      <div className="section-shell">
        <Reveal>
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-8 border-b-2 border-[#1a2f76] pb-4 text-[1.55rem] text-[#112367]">
              {filters.map((item, idx) => (
                <span key={item} className={`inline-flex items-center gap-3 ${idx === 0 ? "font-semibold text-[#1f66ef]" : ""}`}>
